...
def seed():
    with app.app_context():
        ...
        van07 = Vehicle(
            vehicle_number="Van-07",
            vehicle_type="van",
            capacity_kg=500,
            status=Vehicle.STATUS_MAINTENANCE,  # currently in the shop — used to test "unavailable vehicle" rejection
            odometer_km=41030,
        )
        ...
        alex = Driver(
            ...
            license_class="LMV",
            license_expiry=date.today() + timedelta(days=365),  # valid
            phone="9876500001",
        ...
        print(f"  Vehicles:     {Vehicle.query.count()}  (Van-05 id={van05.id})")
        print(f"  Drivers:      {Driver.query.count()}  (Alex id={alex.id})")
        print(f"  Maintenance:  {MaintenanceRecord.query.count()}")
        print(f"  Fuel entries: {FuelExpense.query.count()}")
        print()
       
        print("rejected), new driver marked off_duty (should be rejected), cargo > 500kg")
        print("on Van-05 (should be rejected).")
        print()
